...
import com.iapp.iapp_messenger.lib.util.DataChecks;
...
public class LegacyAccountDAO {
    ...
    private static final Logger accountLogger = LoggerFactory.getLogger(LegacyAccountDAO.class);

    private final LegacyMetaDAO metaDAO;
    ...
    public LegacyAccountDAO(JdbcTemplate jdbcTemplate) {
        metaDAO = new LegacyMetaDAO(jdbcTemplate);
     ...
     * @see LegacyAccountDAO#updateTop()
     * */
    ...
    public Pair<RequestStatus, List<Account>> searchAccounts(String partName) {
        List<Account> searched = jdbcTemplate.query("SELECT * FROM account WHERE username LIKE "
                + String.format("'%%%s%%'", partName), new LegacyAccountMapper());
    ...
    public List<Account> getServerAccount(String name) {
        return jdbcTemplate.query("SELECT * FROM ACCOUNT WHERE name=?",
            new Object[]{name}, new LegacyAccountMapper());
    }
    ...
    public List<Account> getServerAccount(long id) {
        return jdbcTemplate.query("SELECT * FROM ACCOUNT WHERE id=?",
            new Object[]{id}, new LegacyAccountMapper());
    }

    private long getNewID() {
        return jdbcTemplate.query("SELECT * FROM Account ORDER BY id DESC LIMIT 1", new LegacyAccountMapper())
